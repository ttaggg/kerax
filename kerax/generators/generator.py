...
class Generator(utils.Sequence, abc.ABC):
    ...
    def __init__(self, dataset, batch_size, augmentations, is_training):
        ...
        self._augmentation_config = augmentations
        albumentations_dict = {
            ...
            'EdgeDetection': image_utils.EdgeDetection,
        }
    ...
    @property
    def normalize_shift(self):
        """If we subtract mean and divide by std, return tuple (means, stds)."""
        if self._augmentation_config.get('Normalize', None):
            return (np.array(self._augmentation_config['Normalize']['mean']),
                    np.array(self._augmentation_config['Normalize']['std']))
        return None
