...
from loaders import cityscapes_data_loader
from loaders import severstal_data_loader
...
def get_postprocessor(task):
    ...
    if task.endswith('segmentation'):
        ...
        def postprocess_images(images, predictions, batch, output_dir):
            masks = np.argmax(predictions, axis=-1)
            colormap = image_utils.create_label_colormap(masks.shape[-1])
            for i, (image, mask) in enumerate(zip(images, masks)):
                # If image is grayscale, convert RGB for logging.
                if image.shape[-1] == 1:
                ...
                filename = os.path.join(output_dir, 'images',
                                        f'{batch}_{i}.jpg')
                image.save(filename)
        ...
        def postprocess_labels(image, predictions, batch, output_dir):
            del image  # Unused.
            del batch  # Unused.
        ...
        raise ValueError('Only segmentation and classification tasks '
...
def get_loader(config):
    ...
    raise ValueError('Currently only cityscapes and severstal tasks '
